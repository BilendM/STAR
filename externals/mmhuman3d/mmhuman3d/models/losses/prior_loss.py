...
class ShapePriorLoss(nn.Module):
    ...
    def forward(self, betas, loss_weight_override=None, reduction_override=None):
        ...
        loss_weight = loss_weight_override if loss_weight_override is not None else self.loss_weight
...
class ShapeThresholdPriorLoss(nn.Module):
    ...
    def forward(self, betas):
        ...
        if self.norm == "l1":
            return self.loss_weight * invalid_values.abs().sum() / (mask.to(dtype=betas.dtype).sum() + self.epsilon)
        elif self.norm == "l2":
            return self.loss_weight * invalid_values.pow(2).sum() / (mask.to(dtype=betas.dtype).sum() + self.epsilon)
...
class PoseRegLoss(nn.Module):
    ...
    def forward(self, body_pose, weight=None, avg_factor=None, loss_weight_override=None, reduction_override=None):
        reduction = reduction_override if reduction_override else self.reduction
        loss_weight = loss_weight_override if loss_weight_override is not None else self.loss_weight
...
class LimbLengthLoss(nn.Module):
    ...
    def __init__(self, convention, reduction="mean", loss_weight=1.0, eps=1e-4):
        super().__init__()
        assert reduction in (None, "none", "mean", "sum")
        self.reduction = reduction
        self.loss_weight = loss_weight
        ...
        self.limb_idxs = np.array(list(x for x, _ in itertools.groupby(limb_idxs)))
    ...
    def _keypoint_conf_to_limb_conf(self, keypoint_conf):
        limb_conf = torch.min(keypoint_conf[:, self.limb_idxs[:, 1]], keypoint_conf[:, self.limb_idxs[:, 0]])
        return limb_conf

    def forward(
        self, pred, target, pred_conf=None, target_conf=None, loss_weight_override=None, reduction_override=None
    ):
...
class JointPriorLoss(nn.Module):
    ...
    def __init__(
        self, reduction="mean", loss_weight=1.0, use_full_body=False, smooth_spine=False, smooth_spine_loss_weight=1.0
    ):
        super().__init__()
        assert reduction in (None, "none", "mean", "sum")
        self.reduction = reduction
    ...
    def forward(self, body_pose, loss_weight_override=None, reduction_override=None):
        """Forward function of loss.
        ...
        if self.use_full_body:
            ...
            lower_loss = (torch.exp(F.relu(lower_limits - body_pose_sja)) - 1).pow(2)
            upper_loss = (torch.exp(F.relu(body_pose_sja - upper_limits)) - 1).pow(2)

            standard_joint_angle_prior_loss = (lower_loss + upper_loss).view(body_pose.shape[0], -1)  # shape: (n, 3)
            ...
            joint_prior_loss = (
                torch.exp(body_pose[:, [55, 58, 12, 15]] * torch.tensor([1.0, -1.0, -1, -1.0], device=body_pose.device))
                - 1
            ) ** 2
        ...
        if self.smooth_spine:
            ...
            smooth_spine_loss_12 = (torch.exp(F.relu(-spine1 * spine2)) - 1).pow(2) * self.smooth_spine_loss_weight
            smooth_spine_loss_23 = (torch.exp(F.relu(-spine2 * spine3)) - 1).pow(2) * self.smooth_spine_loss_weight

            joint_prior_loss = torch.cat([joint_prior_loss, smooth_spine_loss_12, smooth_spine_loss_23], axis=1)
...
class SmoothJointLoss(nn.Module):
    ...
    def __init__(self, reduction="mean", loss_weight=1.0, degree=False, loss_func="L1"):
        super().__init__()
        assert reduction in (None, "none", "mean", "sum")
    ...
    def forward(self, body_pose, loss_weight_override=None, reduction_override=None):
        ...
        smooth_joint_loss = torch.cat([torch.zeros_like(smooth_joint_loss)[:1], smooth_joint_loss])
...
class SmoothPelvisLoss(nn.Module):
    ...
    def forward(self, global_orient, loss_weight_override=None, reduction_override=None):
        ...
        # add zero padding to retain original batch_size
        smooth_pelvis_loss = torch.cat([torch.zeros_like(smooth_pelvis_loss)[:1], smooth_pelvis_loss]).sum(dim=-1)
...
class SmoothTranslationLoss(nn.Module):
    ...
    def forward(self, translation, loss_weight_override=None, reduction_override=None):
        """Forward function of loss.
        ...
        smooth_translation_loss = translation_diff.abs().sum(dim=-1, keepdim=True)

        # add zero padding to retain original batch_size
        smooth_translation_loss = torch.cat(
            [torch.zeros_like(smooth_translation_loss)[:1], smooth_translation_loss]
        ).sum(dim=-1)
...
class CameraPriorLoss(nn.Module):
    ...
    def forward(self, cameras, loss_weight_override=None, reduction_override=None):
        """Forward function of loss.
...
class MaxMixturePrior(nn.Module):
    ...
    def __init__(
        ...
        if not os.path.exists(full_gmm_fn):
            print('The path to the mixture prior "{}"'.format(full_gmm_fn) + " does not exist, exiting!")
            sys.exit(-1)

...
class LoadImage:
    ...
    def __call__(self, results):
        ...
        if isinstance(results["image_path"], str):
            ...
            img = mmcv.imread(results["image_path"], self.color_type, self.channel_order)
            ...
            raise TypeError('"image_path" must be a numpy array or a str or ' "a pathlib.Path object")
...
def inference_video_based_model(model, extracted_results, with_track_id=True, causal=True):
    ...
    feature_sequences = _collate_feature_sequence(input_features, with_track_id, target_idx)
    ...
    with torch.no_grad():
        results = model(
            features=batch_data["features"], img_metas=batch_data["img_metas"], sample_idx=batch_data["sample_idx"]
        )
...
def _indexing_sequence(input: Union[Sequence, Dict[str, Sequence]], index: Union[int, Tuple[int, ...]]):
...
def _collate_feature_sequence(extracted_features, with_track_id=True, target_frame=0):
    ...
    for idx in range(N):
        ...
        if not with_track_id:
            feature_seq["features"] = np.stack([frame[idx]["features"] for frame in extracted_features])
        else:
